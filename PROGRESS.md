...
- Automated tagged release final-review corrections completed locally (2026-07-24): the producer now emits exact wheel/sdist and separate authoritative `vercor-release-manifest` artifacts; every action is immutable, checkouts drop credentials, publication is serialized per tag, Twine is fixed at 6.2.0, and immediate peeled-tag/public-state checks surround mutations. Ordinary reruns fail on existing PyPI or exact-tag draft/published state; an uninterrupted run verifies PyPI by bounded exact-digest polling, then creates an empty draft, validates the zero/one/two-asset state after each upload, and publishes only the complete draft. Separately authorized documentation binds recovery to one exact tag run and its producer manifest. TDD evidence includes an initial 18 expected failures/3 passes, a scope-correction RED of 2/2 failures, and an extra exact-manifest RED; focused release/API/validator GREEN passed 64/64, fast passed 692/692 (638 deselected), and full passed 1,330/1,330. Black left 237 files unchanged with its known Python-version advisory, flake8 reported 0, mypy passed 237 files, compileall/YAML/all workflow Bash blocks/whitespace passed, and known Flax/JAX and JCM/xarray warnings remained. Action SHAs were resolved read-only from their official repositories. No push, tag, upload, publication, hosted release mutation, or other external mutation was performed.
