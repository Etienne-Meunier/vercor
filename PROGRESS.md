...
- Automated tagged release final-review and re-review corrections completed locally (2026-07-24): the producer emits exact wheel/sdist plus authoritative `vercor-release-manifest` artifacts; immutable actions, credential-free checkouts, per-tag serialization, exact Twine, peeled-tag checks, absence-only ordinary reruns, bounded PyPI verification, and the zero/one/two-asset draft state machine remain enforced. Re-review fixed raw uploads by generating safely encoded canonical `https://uploads.github.com` request targets (the prior `--hostname uploads.github.com` was reproduced targeting invalid `api.uploads.github.com`), made both pre-tag transcripts reject drafts through authenticated paginated enumeration and a malformed/duplicate-safe manifest-free validator, and added bounded exact-digest polling after either manual one-file PyPI recovery. Follow-up TDD RED was exactly 14 failures/17 passes; GREEN passed 31/31 focused and 74/74 release/API/validator, fast 702/702 (638 deselected), and full 1,340/1,340. Black checked 238 files with its known Python-version advisory, flake8 reported 0, mypy passed 237 files, and compileall/YAML/all Bash/whitespace passed; known Flax/JAX and JCM/xarray warnings remained. No push, tag, upload, publication, hosted release mutation, or other external mutation was performed.
