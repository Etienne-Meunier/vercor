...
- Automated tagged release final-review and re-review corrections completed locally (2026-07-24): exact producer artifacts, immutable/credential-free actions, per-tag serialization, exact Twine, peeled-tag gates, absence-only ordinary reruns, bounded PyPI verification, and the zero/one/two-asset draft state machine remain enforced. The final fixes require authenticated repository JSON to prove boolean `permissions.push` true before either pre-tag release enumeration/absence decision, rejecting false/missing/malformed permission state, and commit an installed-`gh` request-construction regression. With a synthetic token and closed loopback proxy, the test proves the old hostname form targets invalid `api.uploads.github.com`, while the production helper's full URL targets `uploads.github.com` and never contacts GitHub or leaks credentials; GH config/state is isolated under `tmp_path`, and the discovered `.local/state/gh/device-id` artifact was removed and remains absent. Exact TDD RED was 8 failures/27 passes, plus one cleanup-side-effect RED; GREEN passed 35/35 exact focus, 80/80 release/API/validator, fast 708/708 (638 deselected), and full 1,346/1,346. Black checked 238 files with its known Python-version advisory, flake8 reported 0, mypy passed 237 files, and compileall/YAML/all Bash/whitespace passed; known Flax/JAX and JCM/xarray warnings remained. No push, tag, upload, publication, hosted release mutation, or other external mutation was performed.
