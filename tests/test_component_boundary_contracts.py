...
def test_public_lifecycle_hook_types_are_owned_by_component_contracts() -> None:
    ...
    for hook_name in removed_hook_names:
        assert f"{hook_name} =" not in contracts_source
    ...
    for path in ("vercor/components/base.py",):
        ...
        assert removed_hook_names.isdisjoint(private_imports), path
        assert "LifecycleHooks" not in public_imports, path
        assert removed_hook_names.isdisjoint(public_imports), path
...
def test_lifecycle_storage_uses_component_spec_as_single_owner() -> None:
    ...
    assert not Path("vercor/components/_protocols.py").exists()
    for dead_module in (
        "_constructor_options.py",
        "_field_authoring.py",
        "_lifecycle_api.py",
        "host.py",
    ):
        assert not Path("vercor/components", dead_module).exists()
