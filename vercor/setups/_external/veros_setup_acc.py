"""Idealized ACC (Antarctic Circumpolar Current channel) Veros setup.

Ported from Veros' built-in ``acc`` example setup (``veros copy-setup acc``),
unmodified apart from removing the standalone-script header. Used as an
alternative, much smaller/cheaper topology to the default global 4-degree
setup -- selected via ``VerosConfig(setup="acc")``.
"""
...
class ACCSetup(VerosSetup):
    ...
    def set_parameter(self, state):
        ...
        # K_gm_0 was moved from Settings to Variables in the differentiable
        # Veros fork (so it can be a traced/differentiable leaf), so it can no
        # longer be assigned here; it defaults to 0.0 on state.variables.K_gm_0.
    ...
    def set_diagnostics(self, state):
        ...
        # Diagnostics (averages, energy, overturning, tracer_monitor) allocate
        # accumulator state lazily on first trigger, which changes the state's
        # PyTree structure partway through a run and breaks the differentiable
        # scanned runtime. Clear them so the payload structure stays fixed.
        diagnostics.clear()
