"""Differentiable global 4-degree Veros setup.

Ported from the ``Veros-Autodiff`` checkout's
``setups/global_4deg/global_4deg_learning.py``, adapted the same way
``acc_learning.py`` was ported into ``veros_setup_acc.py``:

 - ``enable_streamfunction = False`` : avoids the untested/unpatched
   barotropic streamfunction solve path (``solve_stream.py``, island
   line-integral solves). Keeps the model on the ``solve_pressure.py``
   path, which is the one patched with ``lax.cond`` for differentiability.
 - ``eq_of_state_type = 3`` (nonlin2) instead of ``5`` (gsw/TEOS-10) :
   ``gsw.py``'s ``gsw_rho``/``gsw_drhodT``/``gsw_drhodS``/``gsw_drhodP``
   call ``sqrt(salt)`` with no floor, which reproduces the NaN-gradient bug
   ``safe_sqrt`` was built to fix in eke/tke, at every masked/land cell
   (salt == 0). ``nonlinear_eq2.py`` has no sqrt calls at all.
 - All diagnostics disabled (``diagnostics.clear()``) : diagnostics do file
   I/O and build ``xarray.Dataset`` objects from Python-side state, which
   doesn't survive ``jit`` tracing.

Unlike the bundled ``CustomGlobalFourDegree`` (``veros_setup.py``), which
subclasses the plain ``veros.setups.global_4deg.GlobalFourDegreeSetup`` and
expects an external atmosphere to write ``taux``/``tauy``/``qnec``/``qnet``
each step (``VerosConfig(uses_atmosphere_forcing=True)``), this setup forces
itself from its own climatology data files -- select it with
``VerosConfig(setup="global_4deg_learning", uses_atmosphere_forcing=False)``,
mirroring how the ACC setup is run standalone.
"""
...
import veros.setups.global_4deg as _built_in_global_4deg
import veros.tools
from veros import VerosSetup, veros_routine, veros_kernel, KernelOutput, logger
from veros.variables import Variable
from veros.core.operators import numpy as npx, update, at, safe_sqrt

# Reuse the built-in setup's bundled assets.json (identical asset ids/URLs to
# the Veros-Autodiff checkout's), so downloads land in the same
# ``$HOME/.veros/assets/global_4deg`` cache regardless of which setup class
# triggered them.
_BASE_PATH = os.path.dirname(os.path.realpath(_built_in_global_4deg.__file__))
DATA_FILES = veros.tools.get_assets("global_4deg", os.path.join(_BASE_PATH, "assets.json"))


class GlobalFourDegreeLearningSetup(VerosSetup):
    """Global 4 degree model with 15 vertical levels -- differentiable / learning variant.

    Same as the built-in ``global_4deg`` setup, with ``enable_streamfunction``,
    ``eq_of_state_type`` and diagnostics adjusted for gradient computation
    ...
    def set_parameter(self, state):
        ...
        settings.identifier = "global_4deg_learning"
        settings.description = "My global 4 degree learning setup"
...
__all__ = ["GlobalFourDegreeLearningSetup"]
