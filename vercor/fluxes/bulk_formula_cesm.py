...
def old_flux_atmOcn(
    ...
    """Compute atmosphere-ocean surface fluxes using the legacy two-iteration scheme."""
    ...
    al2 = jnp.log(settings.zref / settings.ztref)
    ...
    delq = qbot_array - ssq
    alz = jnp.log(zbot_array / settings.zref)
    cp = settings.cpdair * (1.0 + settings.cpvir * ssq)

    stable = 0.5 + 0.5 * jnp.sign(delt)
    rdn = jnp.sqrt(cdn(vmag))
    rhn = (1.0 - stable) * 0.0327 + stable * 0.018
    ren = jnp.full_like(rdn, 0.0346)
    ...
    hol_2m = hol * settings.ztref / zbot_array
    ...
    tref = (thbot_array - delt * fac - 0.01 * settings.ztref) * mask_array
    qref = (
        qbot_array - delq * (re / settings.karman) * (alz + al2 - psixh + psix2)
    ) * mask_array
