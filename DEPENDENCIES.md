...
24. `vercor/components/base.py` - unified runtime component interface for seed-state creation, contract-driven validation/receive/send, explicit host-runtime adapter contract, and pure stepping built on (8)
...
28. `vercor/coupler.py` unified runtime loop - canonical `run()` / `create_runtime_state()` path, runtime contract ownership derived from exchanges, typed host bridge dispatch, runtime component views, and scanned-runtime rejection for host adapters built on (12, 23, 24, 25, 26, 27)
