...
36. `vercor/runtime/state.py` and `vercor/runtime/__init__.py` - immutable runtime state, coupler-owned component-name metadata, runtime field stores with membership, mapping, fallback, and existing-field replacement helpers, import/export contract construction, pure exchange dispatch, and the internal `vercor.runtime` re-export surface built on (1, 10, 13, 14)
...
39. `vercor/runtime/driver.py` - bundled runtime dispatch context, single per-component runtime step helper with explicit host-runtime allowance, outgoing priming, and host-adapter detection built on (32, 35, 36, 38)
40. `vercor/runtime/coupler_state.py` - runtime coupler-state assembly, contract refresh, topology validation, dispatch-context construction, outgoing priming adapter, and output-mask lookup built on (32, 35, 36, 38, 39)
41. `vercor/runtime/runner.py` - host/scanned runtime loops, callback-safe progress logging, compiled-runtime cache keys, JIT wrapping, donation checks, and interrupt translation built on (30, 31, 37, 39)
42. `vercor/output.py` - runtime-view NetCDF output boundary built on (5, 10, 33, 36)
43. `vercor/components/external/jax_gcm.py` runtime payload path - immutable JCM state/forcing runtime integration built on (7, 32, 35, 36, 38)
44. `vercor/coupler.py` unified runtime facade - canonical `run()` / `create_runtime_state()` path, component registration through the base component contract, exchange-mask/regridder setup, runtime component views, final output, callback-backed logging setup, and thin delegation to runtime-owned state/runner adapters built on (1, 15, 16, 30, 31, 32, 33, 36, 37, 38, 39, 40, 41, 42, 43)
