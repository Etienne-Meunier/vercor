...
24. `vercor/components/base.py` - unified runtime component interface for payload creation, component-owned runtime validation metadata, receive/send, explicit host-runtime adapter contract, and pure stepping built on (8)
25. `vercor/runtime.py` - immutable runtime state, coupler-owned component-name metadata, and pure exchange dispatch built on (8, 10, 11, 24)
...
28. `vercor/coupler.py` unified runtime loop - canonical `run()` / `create_runtime_state()` path, typed host bridge dispatch, runtime component views, and scanned-runtime rejection for host adapters built on (12, 23, 24, 25, 26, 27)
