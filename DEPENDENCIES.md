...
24. `vercor/components/base.py` - unified runtime component interface for seed-state creation, contract-driven validation/receive/send, explicit component init/runtime step contexts, host-runtime adapter contract, and pure stepping built on (8)
25. `vercor/runtime.py` - immutable runtime state, coupler-owned component-name and import/export contract metadata, and pure exchange dispatch built on (8, 10, 11, 24)
26. `vercor/output.py` - runtime-view NetCDF output boundary built on (8, 23, 25)
27. `vercor/components/external/jax_gcm.py` runtime payload path - immutable JCM state/forcing runtime integration built on (5, 24, 25)
28. `vercor/coupler.py` unified runtime loop - canonical `run()` / `create_runtime_state()` path, runtime contract ownership derived from exchanges, minimal component context creation, typed host bridge dispatch, runtime component views, and scanned-runtime rejection for host adapters built on (12, 23, 24, 25, 26, 27)
