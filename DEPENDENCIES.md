...
23. `vercor/components/base.py` - unified runtime component interface for payload creation, component-owned runtime validation metadata, receive/send, explicit host-runtime adapter contract, pure stepping, and runtime-view output built on (8)
...
26. `vercor/coupler.py` unified runtime loop - canonical `run()` / `create_runtime_state()` path, typed host bridge dispatch, runtime component views, and scanned-runtime rejection for host adapters built on (12, 23, 24, 25)
