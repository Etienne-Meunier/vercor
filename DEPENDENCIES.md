...
23. `vercor/components/base.py` - unified runtime component interface for payload creation, state prefill/validation, receive/send, stepping, and wrapper commits built on (8)
24. `vercor/runtime.py` - immutable runtime state and pure exchange dispatch built on (8, 10, 11, 23)
...
26. `vercor/coupler.py` unified runtime loop - canonical `run()` / `create_runtime_state()` path built on (12, 23, 24, 25)
